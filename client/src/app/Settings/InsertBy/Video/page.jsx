...
const Video = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8080/vidURL", {
        method: "POST",
        headers: {
  ...
  return (
    <section className="flex flex-col items-center bg-primaryWhite shadow-md font-poppins py-5 md:w-[35rem] mt-20">
      <div className="flex items-center  w-full justify-center">
        ...
        />
        <h1 className="text-3xl font-normal">Insert Video Source</h1>
      </div>
      <div className="border-[1px] border-black w-full m-4 mb-10" />
      ...
      {success && <p className="text-green-500 text-center mt-5 border-[1.5px] border-green-800 bg-green-50 p-3">{success}</p>}
    </section>
  );
};
